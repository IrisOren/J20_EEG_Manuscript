...
Fig2A<-ggplot(EventData, aes(x = EventHour, fill = Light)) +
...
Figure 2B:
We used circular statistics to establish whether IIS were significantly-coupled to a phase of the circadian cycle within individual animals (Fig 2A, see methods). Indeed X out of Y  animals showed a significant phase coupling, with mean phase=    (Fig 2B, p=   Rayleigh’s test ). 

1. Create Fig2SummaryDataframe from recordingInformation. 
```{r create_fig2_dataframe, ECHO=FALSE}
...
for(i in 1:nrows(Fig2SummaryDataframe)){
  ...
  Fig2SummaryDataframe$SDAngle<-SummaryCircStatsCurrent$SDCurrent[1]
  Fig2SummaryDataframe$RayleighR<-SummaryCircStatsCurrent$RayleighR[1]
  Fig2SummaryDataframe$RayleighP<-SummaryCircStatsCurrent$RayleighP[1]
  AnimalIDCurrent<-c()
  IISTimesCurrent<-c()
