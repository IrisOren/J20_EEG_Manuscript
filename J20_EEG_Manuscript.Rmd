...
SCPP1DataframeCurrent<-filter(SCPP1DataframeAll, 
                              LightDark==TRUE, 
                              Treatment=="Control")
...
for(AnimalIndex in 1:length(AnimalIDList)){
  ...
  ETAv<-rep(0,((WindowLength/8)+1))   #Initialise vector for Event-Triggered average 
  ...
  for(SpikeIndex in 1:NumberOfEvents){
    ...
    ETW<-EventTriggeredWindow(EventTimeCurrent, WindowLength,
                              SCPP1DataframeCurrent$InitialisedTime, 
  ...
  NETAv<-rep(0,((WindowLength/8)+1))   #Initialise vector for NonEvent-Triggered average 
  ...
  for(SpikeIndex in 1:NumberOfNonEvents){
    ...
    NETW<-EventTriggeredWindow(NonEventTimeCurrent, WindowLength, SCPP1DataframeCurrent$InitialisedTime, SCPP1DataframeCurrent$ThetaOverDelta)
  ...
  NETAv<-NETAv/SpikeIndex
  NETList$NETAv[[AnimalIDCurrent]]<-NETAv
  NETList$ETW[[AnimalIDCurrent]]<-NETWList
}
