...
AnimalIDList<-c("JF218")#, "JF220", "JF221", "J0456", "J0460")
ETAvList<-list()
...
for(AnimalIndex in 1:length(AnimalIDList)){
    ...
    if(nrow(ETW)==NoOfIntervalsForAveraging+1){
     ...
     # ETWList[[SpikeIndex]]<-ETW
      ...
      #Append ETSweepsDataframe with current EventTriggeredSweep
        ...
        TimeForAv<-seq(1,NoOfIntervalsForAveraging+1, 1)
  ...
  ETAvDataframeTemp<-data.frame(
    AnimalIDIndex=rep(AnimalIDCurrent, NoOfIntervalsForAveraging+1),
    Time=seq(1, 1+NoOfIntervalsForAveraging, 1),
    AvThetaDelta=ETAv
  )
 ...
 # NETWList<-list()
 # NETAv<-rep(0,NoOfIntervalsForAveraging+1)   #Initialise vector for NonEvent-Triggered average 
...
rm(ETAvDataframeTemp, ETSweepsTemp, ETW)
  
#plot(seq(1, 13, 1), ETList$ETAv[["JF220"]])
# plot(seq(1, 13, 1), ETList$ETAv[["JF221"]])
# plot(seq(1, 13, 1), NETList$NETAv[["JF221"]])
# plot(seq(1, 13, 1), NETList$NETAv[["JF220"]])
