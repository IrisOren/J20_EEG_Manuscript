...
for(AnimalIndex in 1:length(AnimalIDList)){
  ...
  SCPP1DataframeCurrent<-filter(SCPP1DataframeAll, 
                                AnimalIDVariable==AnimalIDCurrent)
  #Calculate ThetaOverDelta for each inte
  SCPP1DataframeCurrent$ThetaOverDelta<-SCPP1DataframeCurrent$Theta/SCPP1DataframeCurrent$Delta
  ...
  for(SpikeIndex in 1:nrow(EventsDataframe)){
    ETW<-EventTriggeredWindow(EventsDataFrame$InitialisedTime[SpikeIndex], WindowLength, SCPP1DataframeCurrent$InitialisedTime, SCPP1DataframeCurrent$ThetaOverDelta)
    ETAv<-ETAv+ETW$TriggeredWindowData
    ETWList[[SpikeIndex]]<-ETW
  }
  ETAv<-ETAv/SpikeIndex
