...
if(file.exists("./Output/SCPPDataframeAllOriginal.csv")){
  SCPPDataframeAll<-read.csv("./Output/SCPPDataframeAllOriginal.csv")
  RemoveOption<-0
      ...
      for(i in 1:nrow(SummaryDataframe)){     #Loop through animals
       ...
       #Next import all files in input_path to create a new r dataframe called             `SCPPDataframe'
        ...
        #Apend a Treatment variable to the dataframe, based on DrugDateTime as specified in Fig1SummaryDataframe 
          Treatment<-c()
          Treatment[which(SCPPDataframe$TimeOfDay<strptime(DrugDateTimeCurrent, "%d/%m/%y %H:%M:%S", tz=TimeZoneCurrent))]="Control"
    ...
    Hour<-lubridate::hour(SCPPDataframeAll$TimeOfDay)
    ...
    SCPPDataframeAll$SingleSpikeCount<-SCPPDataframeAll$SpikeCount
    SCPPDataframeAll$SpikeCount<-SCPPDataframeAll$SingleSpikeCount+SCPPDataframeAll$BurstCount
...
if(file.exists("./Output/SCPPDataframeAll.csv")==0){
        write.csv(SCPPDataframeAll,"./Output/SCPPDataframeAllOriginal.csv")
}
  

```

 exclude intervals from SCPPDataframeAll and save
```{r ExcludeIntervals, echo=FALSE, include=TRUE}

SCPPDataframeAll<-filter(SCPPDataframeAll, ExcludeInterval==0)
if(file.exists("./Output/SCPPDataframeAllExcluded.csv")==0){
          write.csv(SCPPDataframeAll,"./Output/SCPPDataframeAllExcluded.csv")
}
...
DataForTesting <-Fig1SummaryDataframe_WT$Grtr0SpikeCtrl
  SW_WT<-NormTesting(100*DataForTesting, "QQPlot_WT")
  SW_WT
  
DataForTesting <-Fig1SummaryDataframe_J20$Grtr0SpikeCtrl
  SW_J20<-NormTesting(100*DataForTesting, "QQPlot_J20")
  SW_J20
  
#Square root transformation 
  DataForTesting <-sqrt(Fig1SummaryDataframe_WT$Grtr0SpikeCtrl)
  SW_WT_sqrt<-NormTesting(DataForTesting, "QQPlot_WT_sqrt")
  SW_WT_sqrt
  
DataForTesting <-sqrt(Fig1SummaryDataframe_J20$Grtr0SpikeCtrl)
  SW_J20_sqrt<-NormTesting(DataForTesting, "QQPlot_J20_sqrt")
  SW_J20_sqrt
...
```
###T-test to compare intervals with 1 or more spikes between genotypes 
...
Data<-sqrt(Fig1SummaryDataframe$Grtr0SpikeCtrl)
Group<-Fig1SummaryDataframe$Genotype

tTestGrtr0Spike<-t.test(Data~Group)
tTestGrtr0Spike
...
Mean rho: `r mean(Fig2SummaryDataframe_J20$Rho)`
sd rho: `r sd(Fig2SummaryDataframe_J20$Rho)`
...
FileList<-c("./Data/VideoScoring/VideoScoringJF220_040416.csv",
            ...
            #WTs are commented out so as not to have them in the summary plot. However, they can be uncommented to create the individual traces
        ...
        #    "./Data/VideoScoring/VideoScoringJ0373_030816Night.csv",
          ...
          "./Data/VideoScoring/VideoScoringJ0456_250417_1.csv",
          "./Data/VideoScoring/VideoScoringJ0456_250417_1b.csv",
          # "./Data/VideoScoring/VideoScoringJ0456_260417.csv", J0456_260417.csv was for not time. None of the other animals used night time recordings so I am not including this file in analysis
           "./Data/VideoScoring/VideoScoringJ0456_260417_2.csv",
          "./Data/VideoScoring/VideoScoringJ0460_200517.csv",
...
#Copy of SCPPDataframeVideoTimesAll which includes WT
SCPPDataframeVideoTimesAllWithWT<-SCPPDataframeVideoTimesAll

#Copy of SCPPDataframeVideoTimesAll which includes only J20 for use in preexisting code below
SCPPDataframeVideoTimesAll<-filter(SCPPDataframeVideoTimesAll, 
                                   AnimalIDVariable %in% c("JF220", "JF221", "J0456", "J0460"))
rm(SCPPDataframeVideoTimes, VideoScoring)
```

Calculate ratio of REM (theta/delta>2)/total sleep
```{r REM_ratio, include=TRUE, echo=FALSE}
#this chunk uses SCPPDataframeVideoTimesAll with the WT included. For the MS, used WT scored in day time onlu
REMRatioDF<-data.frame(AnimalID=c("JF220", 
                                    "J0373",
                                    "J0375", 
                                    "J0376", 
                                    "JF221",
...
#filter SCPPDataframeVideoTimesAll for sleep periods only
SleepDataframe<-filter(SCPPDataframeVideoTimesAllWithWT, SleepWake=="S")
#Add a variable to SleepDataframe for identifying REM sleep
SleepDataframe$REM<-ifelse(SleepDataframe$ThetaOverDelta>2, "R", "N")


for(i in 1:nrow(REMRatioDF)){
  AnimalIDCurrent<-as.character(REMRatioDF$AnimalID[i])
  SleepdataframeCurrent<-filter(SleepDataframe, AnimalIDVariable==AnimalIDCurrent)
  REMRatioDF$REMTotal[i]<-sum(SleepdataframeCurrent$REM=="R")
  REMRatioDF$SleepTotal[i]<-nrow(SleepdataframeCurrent)
  REMRatioDF$Quantile90[i]<-quantile(SleepdataframeCurrent$ThetaOverDelta, c(0.9))
  REMRatioDF$RangeL[i]<-range(SleepdataframeCurrent$ThetaOverDelta)[1]
  REMRatioDF$RangeU[i]<-range(SleepdataframeCurrent$ThetaOverDelta)[2]
}
REMRatioDF$REM_ratio<-REMRatioDF$REMTotal/REMRatioDF$SleepTotal


REMRatioDFGrouped<-dplyr::group_by(REMRatioDF, Genotype)
REMRatioByGenotype<-dplyr::summarise(REMRatioDFGrouped, 
                      MeanRatio=mean(REM_ratio), 
                      sdRatio=sd(REM_ratio),
                      MeanQuantile90=mean(Quantile90),
                      sdQuantile90=sd(Quantile90),
                      MeanRangeL=mean(RangeL),
                      MeanRangeU=mean(RangeU)
                      )

```
REM to total sleep ratio between genotypes
`r REMRatioByGenotype`
...
PhaseCoupling<-c(rep("Strong", 4), rep("Weak", 4))
...
SleepWakeSpikeCountData$SleepWakeColourMod<-SleepWakeSpikeCountData$SleepWake

SleepWakeSpikeCountData$SleepWakeColourMod[84]<-"X" #tmin=3088
SleepWakeSpikeCountData$SleepWakeColourMod[135]<-"X" #tmin=3496
SleepWakeSpikeCountData$SleepWakeColourMod[254]<-"X" #M1459762631.ndf tmin=848
  ...
  xlim(c(0, 7200))+ 
      ...
      ThetaOverDeltaPlot<-ggplot(SleepWakeSpikeCountData, 
                                 aes(x=ZeroTime, 
                                     y=ThetaOverDelta, 
                                     color=SleepWakeColourMod)) +
        scale_colour_grey(na.value = "red") +
        geom_point(size=PtSize) +
...
ggsave("Output/Figures/Fig4/Fig4A_Revision.pdf",device="pdf",  width=8, height=12, units="cm")      


```


```{r Fig4Examples, include=TRUE}
Filename1<-"./Data/Fig4RawTraces/Tmin3088.txt" #From JF220 M1459759032.ndf

Trace1<-read.csv(Filename1, header = FALSE)
Trace1<-Trace1*400*10^(-6)

Filename2<-"./Data/Fig4RawTraces/Tmin3496.txt" #From JF220 M1459759032.ndf

Trace2<-read.csv(Filename2, header = FALSE)
Trace2<-Trace2*400*10^(-6)

Filename3<-"./Data/Fig4RawTraces/Tmin848.txt" #From JF220 M1459762631.ndf
...
Traces$Time<-seq(from=0,to=((nrow(Trace)-1)/512), by=1/512)
...
Plot<-ggplot(data = Traces.melted, aes(x = Time, y = value))+
    geom_line()+
  facet_grid(variable~.)+
  scale_y_continuous(limits=c(16.5, 18.05))+
  geom_segment(x=0, xend=1, y=16.5, yend=16.5)+ #1s
  geom_segment(x=0, xend=0, y=16.5, yend=17) +  #500microV
  theme_bw()+
  theme(axis.title.x = element_blank(),
         axis.text.x = element_blank(),
         axis.ticks.x = element_blank(),
...
ggsave("Output/Figures/Fig4/Fig4ALeft.pdf",width=12,height=10, units="cm", device="pdf")
```

```{r Fig4ExamplesSpectra, include=TRUE}
Height=4
Width=7
FontSize=10
Filename<-"./Data/Fig4RawTraces/Tmin3088_Spec.txt" #From JF220 M1459759032.ndf

Spec<-read.table(Filename, header = FALSE, sep=" ")
names(Spec)<-c("Freq", "PowerCount2")
Spec$PowerV2e_6<-Spec$PowerCount2*((400*10^(-6))^2/10^(-6))
Plot<-ggplot(Spec, aes(x=Freq, y=PowerCount2))+
  geom_line() +
  coord_trans(x="log10")+
...
ggsave("Output/Figures/Fig4/Fig4Ai_Spec.pdf", device="pdf", height=Height,width=Width, units="cm")

###########

Filename<-"./Data/Fig4RawTraces/Tmin3496_Spec.txt" 
...
ggsave("Output/Figures/Fig4/Fig4Aii_Spec.pdf", device="pdf", height=Height, width=Width, units="cm")

#########
Filename<-"./Data/Fig4RawTraces/Tmin848_Spec.txt" 
...
ggsave("Output/Figures/Fig4/Fig4Aiii_Spec.pdf", device="pdf", height=Height, width=Width, units="cm")
...
```{r, ThetaOverDeltaBySpikeNumChunk, echo=FALSE, include=FALSE}
...
#  geom_point(aes(colour=AnimalIDVariable, #shape=Coupling), size=2.5) +
...
```{r SpikeCountVsThetaOverDelta, echo=FALSE}

SpikesThetaDeltaExport<-dplyr::select(SCPPDataframeControl, SpikeCount, ThetaOverDelta, AnimalIDVariable, InitialisedTime, LightDark)
SpikesThetaDeltaExport<-dplyr::filter(SpikesThetaDeltaExport,LightDark=="TRUE")
SpikesThetaDeltaExport<-dplyr::filter(SpikesThetaDeltaExport, AnimalIDVariable %in% Fig4SummaryDataframe$AnimalID)

SpikeCountVsThetaDelta<-SpikesThetaDeltaExport

SpikeCountVsThetaDelta$TDBin<-cut(SpikeCountVsThetaDelta$ThetaOverDelta, breaks=c(0, 1, 2, Inf),# 3, 4, 5, 6, 7, 8, 9, 10, Inf), 
                labels=c("<1", "1-2", ">2"))#, "3.5", "4.5",
                       #  "5.5", "6.5",  "7.5", "8.5", #"9.5", 
 #                        "10.5"))
TDBin<-as.factor(SpikeCountVsThetaDelta$TDBin)


ThetaDeltaGrouped<-dplyr::group_by(SpikeCountVsThetaDelta, AnimalIDVariable, TDBin)

#Compute the mean spike rate for each animal in each the three bins of theta/delta                    
SummaryThetaDelta<-dplyr::summarise(ThetaDeltaGrouped, MeanSpikeCount=mean(SpikeCount, na.rm = TRUE),
                          CI_L=Rmisc::CI(SpikeCount)[1],
                          CI_U=Rmisc::CI(SpikeCount)[3])

#SummaryThetaDelta<-dplyr::filter(SummaryThetaDelta, AnimalIDVariable %in% Fig4SummaryDataframe$AnimalID)

SummaryThetaDelta$Coupling<-ifelse((SummaryThetaDelta$AnimalIDVariable=="J0456"|SummaryThetaDelta$AnimalIDVariable=="J0460"),"Weak", "Strong")

#SummaryThetaDelta$MeanSpikeCount<-ifelse(SummaryThetaDelta$MeanSpikeCount==0, NaN,SummaryThetaDelta$MeanSpikeCount )
SummaryThetaDelta
...
ThetaDeltaSpikePlot<-ggplot(SummaryThetaDelta, 
  ...
  theme(legend.position = c(0.2, 0.9), 
        ...
        panel.background = element_blank(), 
        axis.line = element_line(colour="black")) +
  xlab(expression(paste(theta, "/", delta))) +
  ylab("Mean spike count/8s") +
  scale_color_brewer(palette="Set1", guide=FALSE) +
  ylim(0, 1.25)
...
rm(ThetaDeltaGrouped, SpikeCountVsThetaDelta)
...
SpikesThetaDeltaExport<-dplyr::filter(SpikesThetaDeltaExport,LightDark=="TRUE")
SpikesThetaDeltaExport<-dplyr::filter(SpikesThetaDeltaExport, AnimalIDVariable %in% Fig4SummaryDataframe$AnimalID)
write.csv(SpikesThetaDeltaExport, "Output/SpikesThetaDeltaExport171117.csv")
...
ETAvFilename<-"Output/ETAvDataframe.csv"
...
if(file.exists(ETAvFilename)==1){
  ...
  ETSweepsDataframe<-read.csv(ETSweepsDataframeFilename)
}else{
  ...
  for(AnimalIndex in 1:length(AnimalIDList)){
    ...
    rm(SCPPDataframeCurrent, ETAv, ETAvDataframeTemp, ETW, SCPPDataframeEventAv)
}
``` 
...
if(file.exists(NETAvFilename)==1){
  ...
  NETSweepsDataframe<-read.csv(NETSweepsDataframeFilename)
}else{
...
#Write Nonevent-triggered average and individual sweeps to csv files
  ...
  rm(NETAv, NETAvDataframeTemp, NETW, SCPPDataframeEventAv, SCPPDataframeCurrent)
}
```
...
ThetaDeltaGrp<-ThetaDeltaGrp[complete.cases(ThetaDeltaGrp),]
 ThetaDeltaGrp<-dplyr::summarise(ThetaDeltaGrp, MeanThetaDelta=mean(ThetaDelta, na.rm=TRUE),
 ...
 #Repeat for Non Events but set CI_U and CI_L to the mean to not plot error bars
    ...
    NEThetaDeltaGrp<-NEThetaDeltaGrp[complete.cases(NEThetaDeltaGrp),]
    NEThetaDeltaGrp<-summarise(NEThetaDeltaGrp,
                               MeanThetaDelta=mean(ThetaDelta, na.rm=TRUE),
                      CI_U=mean(ThetaDelta, na.rm=TRUE),
                   CI_L=mean(ThetaDelta, na.rm=TRUE)
                    )
...
ThetaDeltaGrp$AnimalIDIndex_f<-factor(ThetaDeltaGrp$AnimalIDIndex, levels=c("JF218", "JF220", "JF221", "J0456", "J0460"))
...
ETPlot<-ggplot(ThetaDeltaGrp, aes(x=TimeForAv, y=MeanThetaDelta, color=Event))+
  ...
  geom_errorbar(
    ...
    width=0.2)+
  facet_grid(AnimalIDIndex_f~., scales="free_y")+
  scale_shape_manual(values=c(16,1)) +
  xlab("Time (s)")+
  ylab(expression(paste(theta, "/", delta)))+
...
##########
###IIS contribution to theta/delta

```{r IISContributionToPower, include=TRUE, echo=FALSE}
#First select dataframe as was exported for IIS and theta/delta analysis
IISThetaDeltaDF<-dplyr::select(SCPPDataframeControl, SpikeCount, ThetaOverDelta, AnimalIDVariable, InitialisedTime, LightDark)
IISThetaDeltaDF<-dplyr::filter(IISThetaDeltaDF,LightDark=="TRUE")
IISThetaDeltaDF<-dplyr::filter(IISThetaDeltaDF, AnimalIDVariable %in% Fig4SummaryDataframe$AnimalID)

ThetaOverDeltaValsLong<-data.frame()
for(i in 1:nrow(Fig4SummaryDataframe)){
  AnimalIDCurrent<-Fig4SummaryDataframe$AnimalID[i]
  IISThetaDeltaDFCurrent<-filter(IISThetaDeltaDF, AnimalIDVariable==AnimalIDCurrent)
  #Get non-zero IIS index
  NonZeroIISIndex<-which(IISThetaDeltaDFCurrent$SpikeCount>0)
 
  #We create dataframe which is indexed by the intervals with non-zero IIS
  DataFrameByIISIndex<-data.frame(NonZeroIISIndex)
  
  #For each index, we get the SpikeCount in preceding interval
  for(j in 1:length(NonZeroIISIndex)){
      DataFrameByIISIndex$IISMinus1[j]<-IISThetaDeltaDFCurrent$SpikeCount[NonZeroIISIndex[j]-1]
  }
  
  
  
  #Filter for intervals where IIS-1==0
  DataFrameByIISIndex<-filter(DataFrameByIISIndex, IISMinus1==0)
  
  #Get theta/delta in index
  for(j in 1:nrow(DataFrameByIISIndex)){
      Index<-DataFrameByIISIndex$NonZeroIISIndex[j]
      DataFrameByIISIndex$ThetaOverDelta[j]<-IISThetaDeltaDFCurrent$ThetaOverDelta[Index]
      DataFrameByIISIndex$ThetaOverDeltaMinus1[j]<-IISThetaDeltaDFCurrent$ThetaOverDelta[Index-1]
  }
  
  #Create temporary dataframe of theta/delta values in long format indexed by whether it is interval i-1 or i
  ThetaOverDeltaValsCurrent<-DataFrameByIISIndex[3:4]
  ThetaOverDeltaValsCurrentLong<-reshape2::melt(ThetaOverDeltaValsCurrent, 
                                         variable.name="Index", 
                                         value.name="ThetaOverDelta")
  levels(ThetaOverDeltaValsCurrentLong$Index)<-c("i", "iMinus1")
  ThetaOverDeltaValsCurrentLong$AnimalID<-as.factor(AnimalIDCurrent)
  
  #Bind current thetaOverDelta values to existing long dataframe
  ThetaOverDeltaValsLong<-rbind(ThetaOverDeltaValsLong, ThetaOverDeltaValsCurrentLong)
}
write.csv(ThetaOverDeltaValsLong, "./Output/ThetaOverDeltaValsLong.csv")
rm(ThetaOverDeltaValsCurrent, ThetaOverDeltaValsCurrentLong, DataFrameByIISIndex, IISThetaDeltaDF, IISThetaDeltaDFCurrent)  
```
Now fit linear mixed model, and look at the effect of index on theta over delta
```{r IISContributionToPowerStats, include=TRUE, echo=FALSE}

ThetaOverDeltaGrouped<-group_by(ThetaOverDeltaValsLong, 
                                AnimalID, Index)
ThetaOverDeltaGrouped<-dplyr::summarise(ThetaOverDeltaGrouped, 
                                        MeanThetaOverDelta=mean(ThetaOverDelta))
ThetaOverDeltaGrouped


ThetaOverDelta.model<-lme4::lmer((ThetaOverDelta)^(1/4)~ Index + (1|AnimalID),
                           data=ThetaOverDeltaValsLong, 
                           REML=FALSE)

#Plot residuals
plot(fitted(ThetaOverDelta.model), residuals(ThetaOverDelta.model))
qqnorm(residuals(ThetaOverDelta.model))
hist(residuals(ThetaOverDelta.model))
summary(ThetaOverDelta.model)


#Test signficance of SpikesBinary fixed effect using a chi-squared statistic
ThetaOverDelta.null<-lme4::lmer((ThetaOverDelta)^(1/4)~ (1|AnimalID), 
                          data=ThetaOverDeltaValsLong,
                          REML = FALSE)
anova(ThetaOverDelta.model, ThetaOverDelta.null)

coef(summary(ThetaOverDelta.model))
confint(ThetaOverDelta.model)
```
The statistics table in the manuscript reports a 95%CI for the slope as 1.96*SEM. Here 0.0018 +-1.96*0.003=(-0.00408, 0.00768)


#####################

Example of theta/delta and spike count for WT
```{r ThetaDeltaExampleWT, echo=FALSE}

AnimalIDCurrent<-"J0373"
StartTime<-AnimalIDVideoStartEnd$J0373_6[[2]]-14400
EndTime<-AnimalIDVideoStartEnd$J0373_6[[2]]-7200
#Note, the subtraction of points is not based on anything in the data, other than to capture both wake and sleep epochs

SleepWakeSpikeCountDataWT<-filter(SCPPDataframeVideoTimesAllWithWT, InitialisedTime>StartTime & InitialisedTime<EndTime & AnimalIDVariable==AnimalIDCurrent)

SleepWakeSpikeCountDataWT$ZeroTime<-SleepWakeSpikeCountDataWT$InitialisedTime-min(SleepWakeSpikeCountDataWT$InitialisedTime)

SleepWakeSpikeCountDataWT$SleepWakeColourMod<-SleepWakeSpikeCountDataWT$SleepWake

SleepWakeSpikeCountDataWT$SleepWakeColourMod[151]<-"X" #M1470125501 tmin=3472
SleepWakeSpikeCountDataWT$SleepWakeColourMod[651]<-"X" #M1470132701 tmin=312

SleepWakeSpikeCountDataWT$SleepWakeColourMod[771]<-"X" #M1470132701 tmin=1280


###
SleepWakeSpikeCountDataWT$ThetauVolts<-SleepWakeSpikeCountDataWT$Theta*160


SleepWakeSpikeCountDataWT$DeltauVolts<-SleepWakeSpikeCountDataWT$Delta*160
...
ThetaPlot<-ggplot(SleepWakeSpikeCountDataWT, 
                        aes(x=ZeroTime, 
                            y=ThetauVolts, 
                            color=SleepWakeColourMod)) +
        scale_colour_grey() +
        geom_point(size=PtSize) + 
        scale_y_continuous(limits=c(0,10000), breaks=seq(0, 10000, 5000))+
        xlab("Time (s)") +
  ylab(expression(paste(theta, " (", mu, V^{2}, ")"))) +
  xlim(c(0, 7200))+ 
        theme(axis.title.x=element_blank(), 
...
DeltaPlot<-ggplot(SleepWakeSpikeCountDataWT, 
                        aes(x=ZeroTime, 
                            y=DeltauVolts, 
                            color=SleepWakeColourMod)) +
        scale_colour_grey() +
...
ThetaOverDeltaPlot<-ggplot(SleepWakeSpikeCountDataWT, 
                                 aes(x=ZeroTime, 
                                     y=ThetaOverDelta, 
                                    color=SleepWakeColourMod))  +
        scale_colour_grey(na.value = "red") +
  ...
  xlab("Time (s)")
    ...
    SleepWakeThetaDelta<-plot_grid(DeltaPlot,ThetaPlot,  ThetaOverDeltaPlot, align = "v", nrow = 3, rel_heights = c(1,1,1.25))
  
 SleepWakeThetaDelta
 

 
ggsave("Output/Figures/FigWTECoG/FigWTECoG_Revision.pdf",device="pdf",  width=8, height=10,  units="cm")      


```


```{r FigWTExamples, include=TRUE}
Filename1<-"./Data/Fig4RawTraces/J0373/M1470125501_Tmin3472.txt"

Trace1<-read.csv(Filename1, header = FALSE)
Trace1<-Trace1*400*10^(-6)

Filename2<-"./Data/Fig4RawTraces/J0373/M1470132701_Tmin312.txt"
Trace2<-read.csv(Filename2, header = FALSE)
Trace2<-Trace2*400*10^(-6)

Filename3<-"./Data/Fig4RawTraces/J0373/M1470132701_Tmin1280.txt" 

Trace3<-read.csv(Filename3, header = FALSE)
Trace3<-Trace3*400*10^(-6)
TracesWT<-data.frame(c(Trace1, Trace2, Trace3))
names(TracesWT)<-c("i. Wake", "ii.Sleep", "iii.Sleep")

TracesWT$Time<-seq(from=0,to=((nrow(Trace)-1)/512), by=1/512)



TracesWT.melted<-melt(TracesWT, id="Time")

PlotWT<-ggplot(data = TracesWT.melted, aes(x = Time, y = value))+
    geom_line()+
  facet_grid(variable~.)+
  scale_y_continuous(limits=c(16.5, 18.05))+
  geom_segment(x=0, xend=1, y=16.5, yend=16.5)+ #1s
...
ggsave("Output/Figures/FigWTECoG/FigWTSignals.pdf",width=12,height=10, units="cm", device="pdf")
```
...
Width=7
FontSize=10
Filename<-"./Data/Fig4RawTraces/J0373/M1470125501_Tmin3472_Spec.txt" 
...
ggsave("Output/Figures/FigWTECoG/FigWTi_Spec.pdf", device="pdf", height=Height,width=Width, units="cm")

###########

Filename<-"./Data/Fig4RawTraces/J0373/M1470132701_Tmin312_Spec.txt"
...
ggsave("Output/Figures/FigWTECoG/FigWTii_Spec.pdf", device="pdf", height=Height, width=Width, units="cm")

#########
Filename<-"./Data/Fig4RawTraces/J0373/M1470132701_Tmin1280_Spec.txt" 
...
ggsave("Output/Figures/FigWTECog/FigWTiii_Spec.pdf", device="pdf", height=Height, width=Width, units="cm")
...
WTThioclineRate<-filter(AChEAbsorptionGenotype,Genotype=="WT")  
J20ThioclineRate<-filter(AChEAbsorptionGenotype,Genotype=="J20")
...
coef(summary(AChEAbsorption.model))
confint(AChEAbsorption.model)
...
GLHTSummary<-summary(glht(AChEAbsorption.model, linfct = mcp(GenotypeTreatment = "Tukey"), alternative = "two.sided", test = adjusted("holm")))
...
```
The statistics table in the manuscript reports the 95%CI for  genotype as slope+-1.96SEM for Genotype = 6.667+-1.96*521.619
