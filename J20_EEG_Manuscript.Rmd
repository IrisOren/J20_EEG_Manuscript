...
2. For each AnimalID:
  ...
  iv. Create data frame with empirical cumulative distribution for each animal
  
  
```{r CalculateSpikeRateChunk, ECHO=FALSE, include=FALSE}
...
for(i in 1:nrow(fig1_summary_dataframe)){
  ...
  #Apend time information to data frame  
    ...
    #Apend treatment variable  TO DO!!!!!!!!!!
    Treatment<-c()
    Treatment[which(get(animalID_current)$TimeOfDay<strptime(drug_date_time, "%Y-%m-%d %H:%M:%S"))]="Control"
Treatment[length(Treatment)+1]="Unknown"
Treatment[(length(Treatment)+1):(dim(get(animalID_chan))[1])]="Drug"
assign(animalID_chan, cbind(get(animalID_chan), Treatment))
...
#create ECDF for animal i
  ECDF_current <-CreateECDF(SCPP1_dataframe, genotype_current, animalID_current)
  ECDF_list[[animalID_current]]<-ECDF_current
}
```

## Compare ECDFs between animals
...
ecdf_df_J20<-filter(ECDF_df_all, Genotype==2)
ecdf_df_WT<-filter(ECDF_df_all, Genotype==1)
