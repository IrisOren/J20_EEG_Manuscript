...
FalsePositives<-select(FalsePositives, AnimalID, FalsePositiveRateWithNa, FalsePositiveRateNoNa)
...
LossIntervalPercentage<-select(LossIntervalPercentage, AnimalID, LossIntervalPercentage)
...
rm(SCPPDataframe, FalsePositives, LossIntervalPercentage, Temp)
```
...
Make a copy of SCPPDataFrameAllOriginal and exclude intervals from SCPPDataframeAll
...
__________
Calculate percentage change of spikes between first 0-24 hours and 48-72hours

```{r CalcSpikeRateForEachDay, include=TRUE, echo=FALSE}
MeanSpike1<-c()
MeanSpike2<-c()
MeanSpike3<-c()
SDSpikeCount<-c()
for(i in 1:nrow(SummaryDataframe)){
  AnimalIDCurrent<-SummaryDataframe$AnimalID[i]
  DataframeTemp<-filter(SCPPDataframeAll, AnimalIDVariable==AnimalIDCurrent)
  DataframeTemp<-filter(DataframeTemp, ExcludeInterval==0)
  MeanSpike1[i]<-mean(DataframeTemp$SpikeCount[1:(450*24)], na.rm=TRUE)
  MeanSpike2[i]<-mean(DataframeTemp$SpikeCount[(450*24)+1:(450*48)],na.rm=TRUE)
  MeanSpike3[i]<-mean(DataframeTemp$SpikeCount[(450*48)+1:(450*72)], na.rm=TRUE)
  SDSpikeCount[i]<-sd(DataframeTemp$SpikeCount[(450*48)+1:(450*72)], na.rm=TRUE)
  
}

SummaryDataframe<-cbind(SummaryDataframe, MeanSpike1, MeanSpike2, MeanSpike3, SDSpikeCount)
SummaryDataframe$Change2<-2*(SummaryDataframe$MeanSpike2-SummaryDataframe$MeanSpike3)/(SummaryDataframe$MeanSpike2+SummaryDataframe$MeanSpike3)

SummaryDataframe$Change1<-(SummaryDataframe$MeanSpike1-SummaryDataframe$MeanSpike2)/SDSpikeCount
```
...
SummaryStats<-SummarySE(Fig1SummaryDataframe, measurevar="Grtr0SpikeCtrl", groupvars=c("Genotype"), na.rm=TRUE)

SummaryStats$CI_L<-SummaryStats$Grtr0SpikeCtrl-SummaryStats$ci
SummaryStats$CI_U<-SummaryStats$Grtr0SpikeCtrl+SummaryStats$ci
...
IISTimesExample<-select(IISTimesExample, SpikeCount, TimeOfDay)
...
```{r create_fig2_dataframe, ECHO=FALSE, include=FALSE}
  Fig2SummaryDataframe <- SummaryDataframe[, c("AnimalID", "Genotype", "TimeZone", "Fig2_CircadianCoupling", "LossIntervalPercentage")]
#Remove rows with missing variables
...
```
Across the sample of J20s, the IIS was significantly coupled to light periods (Phi_J20=`r SampleMeanDateTime$TimeOfDay`), Rho_20=`r SampleMeanDeviation`, RayleighP_J20=`r SampleRayleighP`.
...
filename<-"./Output/SummaryRadialStatsControl.jpeg"
 ...
 RadialPlot<-radial.plot(Fig2SummaryDataframe_J20$Rho,
                                Fig2SummaryDataframe_J20$MeanAngle,
                                start=(pi/2), clockwise = TRUE,
                                labels=c("0/24h","6h", "12h", "18h"),
                                label.pos = c(0, (pi/2), pi, (3*pi/2)), 
...
for(FileIndex in 1:length(FileList)){
   ...
   AnimalIDCurrent<-AnimalIDList[[FileIndex]]
    ...
    VideoScoring<-select(VideoScoring, DateOfEEG, TimeOfEEG,  SleepWake)
...
SummarySW<-summarise(SW_grouped,
...
SWPlot<-ggplot(SummarySW, 
               aes(x=SleepWake, 
                   y=MeanSpikeRate)) +
                 facet_grid(.~AnimalIDVariable) +
  geom_line(aes(color=AnimalIDVariable)) +
  geom_point(aes(color=AnimalIDVariable)) +
  geom_errorbar(aes(ymin=MeanSpikeRate-SEM, ymax=MeanSpikeRate+SEM, width=0.4))+
  ylab("Mean spike/s") +
  xlab("Behavioural State") +
  theme(legend.position = "none")
...
```{r create_fig3_dataframe, ECHO=FALSE}
  Fig3SummaryDataframe<-SummaryDataframe[, c("AnimalID", "Genotype", "TimeZone", "DrugDateTime", "Fig3_BrainState", "LossIntervalPercentage")]
#Remove rows with missing variables
...
Fig3SummaryDataframe<-filter(Fig3SummaryDataframe, Fig3_BrainState=="YES", Genotype=="J20", LossIntervalPercentage<LossIntervalPercentageThreshold  )
...
#Paired t-test AM
Fig5SummaryDataframeAM$SpikeRateDif<-Fig5SummaryDataframeAM$Control-Fig5SummaryDataframeAM$DPZ

#Test assumptions
SW_DPZAM<-NormTesting(Fig5SummaryDataframeAM$SpikeRateDif, "QQ_DPZAM")
SW_DPZAM

TTestAM<-t.test(Fig5SummaryDataframeAM$Control, Fig5SummaryDataframeAM$DPZ, 
                      paired = TRUE)


#Paired t-test PM
Fig5SummaryDataframePM$SpikeRateDif<-Fig5SummaryDataframePM$Control-Fig5SummaryDataframePM$DPZ

#Test assumptions
SW_DPZPM<-NormTesting(Fig5SummaryDataframePM$SpikeRateDif, "QQ_DPZPM")
SW_DPZPM


TTestPM<-t.test(Fig5SummaryDataframePM$Control, Fig5SummaryDataframeAM$DPZ, 
                      paired = TRUE)
