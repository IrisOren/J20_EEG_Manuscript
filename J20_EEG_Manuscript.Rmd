...
for(i in 1:nrow(Fig1SummaryDataframe)){
  ...
  Fig1SummaryDataframe$Grtr1SpikeCtrl[i]<-1-(ECDFCurrent$CumFrequency[2]) 
}
...
SummarySE(Fig1SummaryDataframe, measurevar="Grtr1SpikeCtrl", groupvars=c("Genotype"), na.rm=TRUE)
...
DataForTesting <-Fig1SummaryDataframe_WT$Grtr1SpikeCtrl
  SW_WT<-NormTesting(DataForTesting, "QQPlot_WT")
  
DataForTesting <-Fig1SummaryDataframe_J20$Grtr1SpikeCtrl
...
```{r Grtr1spike_PermutationTest_Chunk, echo=TRUE}



#Compare PercGreaterOneSpike in WT and J20

Data<-Fig1SummaryDataframe$Grtr1SpikeCtrl
...
```
The number of intervals with one or more spikes was compared between genotypes using a permutation test. There was a significant difference between genotypes in the % intervals with 1 or more spikes (p=`r pCE`, permutation test with complete enumeration)
...
stripchart(Grtr1SpikeCtrl ~ Genotype, data=Fig1SummaryDataframe,
