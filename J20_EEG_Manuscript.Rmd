...
summarise(GroupedData, mean=mean(Grtr1SpikeCtrl, na.rm=TRUE), sd=sd(Grtr1SpikeCtrl, na.rm=TRUE))
```
...
PrintVector<-c(pCE, pEMC,pEN, pPCLT)
cat("pCE   pEMC    pEN    pPCLT:  ")
cat(PrintVector)
cat("pCE (exact using complete enumeration) good for small samples")
