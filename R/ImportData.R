...
ImportData<-function(data_dir){
  ...
  FileList<-list.files(path=data_dir, pattern="SCPP3V2.txt") #List of all files in working directory
  ...
  for (i in 1:EndFile){
    ...
    names(DF_temp)<-c("FileName", "Seconds", "Chan", "Loss", "SpikeCount", "Delta", "Theta")
    #Remove columns with null data
    DF_temp<-select(DF_temp, FileName, Seconds, Chan, Loss, SpikeCount, Delta, Theta) 
